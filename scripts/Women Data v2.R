...
# hints from http://www.r-bloggers.com/making-data-visually-appealing/
p1 <- ggplot(df.c, aes(x=Region,y=YesCount)) +
  geom_boxplot(aes(fill=Region),alpha=I(0.5)) + 
  geom_jitter(position=position_jitter(width=.2), size=2,alpha=0.2) 
 
# hint from http://www.r-bloggers.com/my-favorite-graphs/
p2 <- ggplot(df.c, aes(x=Region,y=YesCount)) +
  geom_jitter(position=position_jitter(width=.2), size=2,alpha=0.2) +
  geom_boxplot(aes(fill=Region),alpha=I(0.5)) +
  coord_flip()

p3 <- ggplot(df.c, aes(x=Region,y=YesCount)) +
  geom_jitter(position=position_jitter(), size=2,colour="lightblue",alpha=0.7) +
